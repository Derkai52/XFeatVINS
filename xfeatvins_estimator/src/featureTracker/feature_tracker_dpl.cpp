...
void FeatureTrackerDPL::initializeExtractorMatcher(int extractor_type_, string &extractor_weight_path, string &matcher_weight_path, float matcher_threshold = 0.5)
{
    ...
    FeatureExtractorDPL = std::make_shared<XFeat>();
    FeatureExtractorDPL->initialize(extractor_weight_path);

    // 初始化特征匹配 LightGlue 模型
...
void FeatureTrackerDPL::extract_features_dpl(cv::Mat img, vector<cv::Point2f> &pts, vector<pair<cv::Point2f, vector<float>>> &dplpts_descriptors, int max_num, double extractor_threshold, int radius, cv::Mat &mask)
{
    ...
    for (int i = 0; i < keypoints1.size(0); ++i) {
        float x = keypoints1[i][0].item<float>();
        float y = keypoints1[i][1].item<float>();
        cv::Point2f p(x, y);
        if (!inBorder(p))
            continue;
    ...
    if (1)
    {
        ...
        if(!USE_GPU)
        {
            if (n_max_cnt > 0)
            {
                ...
                // 输出时间到控制台
                std::cout << "Duration11: " << duration.count() << " seconds" << std::endl;
            }
            else
